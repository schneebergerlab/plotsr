...
gzip -d eri.filtered.fa.gz
gzip -d kyo.filtered.fa.gz
gzip -d ler.filtered.fa.gz
gzip -d sha.filtered.fa.gz
...
syri -c cvi_eri.bam -r cvi.filtered.fa -q eri.filtered.fa -F B --prefix cvi_eri &
syri -c eri_sha.bam -r eri.filtered.fa -q sha.filtered.fa -F B --prefix eri_sha &
syri -c sha_kyo.bam -r sha.filtered.fa -q kyo.filtered.fa -F B --prefix sha_kyo &
syri -c kyo_an1.bam -r kyo.filtered.fa -q an1.filtered.fa -F B --prefix kyo_an1 &
syri -c an1_c24.bam -r an1.filtered.fa -q c24.filtered.fa -F B --prefix an1_c24 &
